...
import { Bookmark01Icon, BookmarkAdd01Icon, BookmarkMinus01Icon, Cancel01Icon, FilterIcon, MinusSignCircleIcon, Tick01Icon } from "@hugeicons/core-free-icons";
  ...
  return (
    ...
    >
      ...
      {!filterElement.props.alwaysOn && (
        ...
        >
          <HugeiconsIcon icon={MinusSignCircleIcon} className="h-4 w-4" />
        </Button>
      )}
    </div>
  );
    ...
    <div className={cn("inline-block", className)} {...rest}>
      <DropdownMenu open={open} onOpenChange={setOpen}>
        <DropdownMenuTrigger asChild>
          <Button
            type="button"
            className={cn("add-filter", minimal && "!h-6 !w-6 !p-0")}
            variant={variant}
            size={minimal ? "icon" : size}
            aria-haspopup="true"
            aria-label={translate("ra.action.add_filter")}
          >
            <HugeiconsIcon icon={FilterIcon} className={minimal ? "h-3.5 w-3.5" : "h-4 w-4"} />
            {!minimal && translate("ra.action.add_filter")}
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start" className="w-56">
          ...
          {validSavedQueries.map((savedQuery: SavedQuery, index: number) =>
            isEqual(savedQuery.value, {
              ...
              <DropdownMenuItem
                ...
                key={index}
              >
                <HugeiconsIcon icon={BookmarkMinus01Icon} className="h-4 w-4 mr-2" />
              ...
              <DropdownMenuItem
                ...
                key={index}
              >
                <HugeiconsIcon icon={Bookmark01Icon} className="h-4 w-4 mr-2" />
                {savedQuery.label}
              </DropdownMenuItem>
            ),
          )}
          {hasFilterValues && !hasSavedCurrentQuery && !disableSaveQuery && (
            <DropdownMenuItem onClick={showAddSavedQueryDialog}>
              <HugeiconsIcon icon={BookmarkAdd01Icon} className="h-4 w-4 mr-2" />
  ...
  return (
    ...
    >
      <div className="flex items-center justify-center w-4 h-4 mr-2">
        {displayed && <HugeiconsIcon icon={Tick01Icon} className="h-3 w-3" />}
      </div>
      <div>
        <FieldTitle
