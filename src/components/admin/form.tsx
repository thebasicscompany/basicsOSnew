...
import { FloppyDiskIcon, Loading02Icon } from "@hugeicons/core-free-icons";
...
const SaveButton = <RecordType extends RaRecord = RaRecord>(
  ...
  return (
    <Button
      ...
      {...rest}
    >
      {isSubmitting ? <HugeiconsIcon icon={Loading02Icon} className="mr-2 h-4 w-4 animate-spin" /> : icon}
      {displayedLabel}
    </Button>
  );
};

const defaultIcon = <HugeiconsIcon icon={FloppyDiskIcon} className="h-4 w-4" />;
