---
title: Inbound Email
description: This document explains how to use inbound email in Basics CRM.
---

Basics CRM can receive inbound emails and automatically create or update contacts based on the email content. This feature is useful to keep your CRM up-to-date with the latest interactions with your contacts.
...
Send of forward emails to Basics CRM's inbound email address to automatically add notes to the corresponding contacts. You can find this email address in the user menu by clicking on the "My Info" menu item, in the "Inbound Email" section. It looks like `xxxxxxx@inbound.postmarkapp.com`.

![Inbound Email Address](../../images/inbound-email.jpg)

When you write to a contact in your email client, add the inbound email address in the **Bcc** field. PPostmark will receive a copy of your email, and forward it to Basics CRM. Basics CRM uses the sender's email address to identify the user, and the recipient's email address to identify the contact. If the contact does not yet exist, Basics CRM creates it using the email's domain as company name. The content of the email is added as a note to the contact.

**Tip**: Use the **Bcc** field to send emails to Basics CRM without notifying the recipients.
...
When receiving this email, Basics CRM will:
