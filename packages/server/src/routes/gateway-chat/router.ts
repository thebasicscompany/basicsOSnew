...
export async function routeChatRequest(args: {
  ...
  const prompt = [
    ...
    '- Use "crm_context" for org-specific read questions like summaries, recent items, pipeline status, hot contacts, or questions that depend on CRM facts but not mutations.',
    '- Use "tool_call" for anything that should search specific records, list records for an entity, or create/update/complete/add records, tasks, or notes.',
