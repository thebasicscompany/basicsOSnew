 ...
 */
export function createAirtableImportRoutes(
  db: Db,
  ...
  app.post("/sync", async (c) => {
    ...
    if (!body.baseId || !body.airtableKey) {
      return c.json(
        { error: "baseId and airtableKey are required" },
        400,
      );
    }
    ...
    return nocoProxy(
      `/api/v2/meta/bases/${env.NOCODB_BASE_ID}/syncs`,
      "POST",
      syncBody,
    );
  });
  ...
  app.post("/trigger", async (c) => {
    const { syncId } = await c.req.json<{ syncId: string }>();
    if (!syncId) {
