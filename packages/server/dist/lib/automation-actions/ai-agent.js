...
export async function executeAIAgent(config, _context, db, salesId, apiKey, env) {
            ...
            getContacts: tool({
                ...
                execute: async ({ query }) => {
                    return db
                        ...
                        .where(or(like(schema.contacts.firstName, `%${query}%`), like(schema.contacts.lastName, `%${query}%`), like(schema.contacts.email, `%${query}%`)))
                        .limit(10);
                },
            }),
            getDeals: tool({
