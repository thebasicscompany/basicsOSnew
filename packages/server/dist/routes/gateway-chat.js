...
export function createGatewayChatRoutes(db, auth, env) {
    ...
    app.post("/", authMiddleware(auth), async (c) => {
        ...
        const apiKey = c.req.header("X-Basics-API-Key")?.trim() || sale.basicsApiKey;
        if (!apiKey) {
            return c.json({ error: "Basics API key not configured. Add your key in Settings." }, 400);
        }
        let body;
        try {
        ...
        if (!gatewayRes.ok || !gatewayRes.body) {
            ...
            console.error("[gateway-chat] gateway error:", gatewayRes.status, errText);
