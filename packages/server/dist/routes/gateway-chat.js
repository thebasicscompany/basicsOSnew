...
fullConversationText) {
    ...
    if (isMeetingFollowUp) {
        ...
        planLines.push(`This is a post-meeting follow-up for meeting_id ${mid}. Required sequence:`, `1. Call \`link_meeting_to_contact\` with meeting_id=${mid} and the contact name/id the user provides — pass contact_name directly, no search needed first.`, `2. If the user mentions action items or tasks, call \`create_task\` for each one.`, `3. Confirm everything was saved. Do not reply before completing all tool calls.`);
    ...
    if (isTask) {
        planLines.push("For task requests, the usual sequence is: identify the contact or company first if needed, then call `create_task`, then confirm the task was created.");
    ...
    if (isNote && /\bdeal|deals|opportunity|opportunities\b/.test(lower)) {
        ...
        planLines.push("For note requests about a contact, the usual sequence is: identify the contact first if needed, then call `create_note` or `add_note`, then confirm the note was added. Always include a short title.");
    ...
    if (isCreateContact && /\bcompany|companies|organization\b/.test(lower)) {
        planLines.push("When creating a contact with a company name, pass company_name directly to create_contact. Do NOT search for the company first — the tool auto-creates the company if it does not exist.");
    ...
    if (isCreateDeal && /\bcompany|companies|organization\b/.test(lower)) {
        planLines.push("When creating a deal with a company name, pass company_name directly to create_deal. Do NOT search for the company first — the tool auto-creates the company if it does not exist.");
        // No nextHintByTool for search_companies — we skip the search step entirely
    }
    const isBulkCreate = (/\b(and also|and then|also|too)\b|,/.test(lower) &&
    ...
    if (isBulkCreate) {
        planLines.push("For bulk create requests (multiple contacts, companies, or deals in one message), call one create tool per record in sequence, then confirm all were created.", "Do not stop after the first record. Complete every create action before replying.");
    }
